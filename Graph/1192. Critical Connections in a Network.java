...
// Solution: Tarjan's Algorithm For Bridge, TC: O(Edges*(Vertices+Edges))

// Intuition -
// An edge (u,v) will be a critical connection, if to reach node v in the network we've only node u, i.e. if we start visiting nodes from a source vertex 0 using DFS, and keep assigning discovery time to each time, while checking if it can be visited earlier from any alternate route. If it's possible to visit a node earlier from different route, than this node cannot be a bridge/critical connection.

// If it's node possible to visit a node v earlier from a different route than the current parent node u, then the edge (u,v) is critical connection.
