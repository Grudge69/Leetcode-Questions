...
class Solution {
    ...
    private ArrayList<Integer> helper(TreeNode root, int d){
        ...
        //from those answers check which are good leaf node pairs i.e. sum of distance from left to right <=distance
        for(int a: leftAns){
            for(int b: rightAns){
                if(a!=0 && b!=0 && a+b<=d){
        ...
        //for left subtree's ans, whichever value+1<distance then it can be summed with another value from right subtree with 1 added to it for going 1 lvl above
        for(int a: leftAns){
            //the distance should not be zero as this is the distance where node doesn't exist
            if(a!=0 && a+1<d){
        ...
        //for right subtree's ans, whichever value+1<distance then it can be summed with another value from left subtree with 1 added to it for going 1 lvl above
        for(int b: rightAns){
            //the distance should not be zero as this is the distance where node doesn't exist
            if(b!=0 && b+1<d){
                //add the values in our list which are allowed to go to above level
