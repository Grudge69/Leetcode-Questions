...
class Solution{
    ...
    public Tree cloneTree(Tree tree){
       ...
       Tree root = helper(tree, map);
       
       return root;
     }
     
    private Tree traverse(Tree root, HashMap<Tree, Tree> map) {
        if(root == null) {
            return null;
        }
        ...
        node.left = traverse(root.left, map);
        node.right = traverse(root.right, map);
        
        return node;
    }
    
    private Tree helper(Tree root, HashMap<Tree, Tree> map) {
        if(root == null) {
            return null;
        }
        
        Tree node = map.get(root);
        node.left = helper(root.left, map);
        node.right = helper(root.right, map);
